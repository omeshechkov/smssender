...
INSERT INTO `state#short_message`(id, value) values (5, 'Delivered');
INSERT INTO `state#short_message`(id, value) values (6, 'Expired');
INSERT INTO `state#short_message`(id, value) values (7, 'Deleted');
INSERT INTO `state#short_message`(id, value) values (8, 'Undeliverable');
INSERT INTO `state#short_message`(id, value) values (9, 'Accepted');
INSERT INTO `state#short_message`(id, value) values (10, 'Rejected');
INSERT INTO `state#short_message`(id, value) values (11, 'Unknown');
COMMIT;
...
insert into short_message(`id`, dispatching_uid, sequence, state, message_id, end_position)
  SELECT (@seq := @seq + 1) AS `id`, d.uid, 0, d.state, d.message_id, char_length(d.message) - 1 from dispatching d;
COMMIT;

insert into short_message_state(id, state, message_id, smpp_status, timestamp)
	select sm.id, ds.state, ds.message_id, ds.smpp_status, ds.`timestamp` from dispatching_state ds
...
BEGIN
  ...
  if v_short_message_current_sequence = v_current_sequence then
    update dispatching d
...
BEGIN
  ...
  insert into short_message_state(`id`, message_id, state, `timestamp`)
    values(p_id, v_message_id, p_state, p_timestamp);
end
$$
...
BEGIN
  ...
  select sm.dispatching_uid, sm.`id`, sm.message_id into v_operation_uid, v_short_message_id, v_message_id
    from short_message sm
...
root:BEGIN
  ...
  if p_operation_type_id = 10 then -- replace
    ...
    where t.destination_number = p_destination_number
      ...
      and ((t.state = 0) or (t.state = 2 and t.total_messages != t.messages_delivered));
  ...
  insert into `dispatching` (`uid`, `operation_type_id`, `source_number`, `source_number_ton`, `source_number_npi`, `destination_number`, `service_type`, `message`, `service_id`, `state`, `current_seq`)
    values (p_uid, p_operation_type_id, p_source_number, p_source_number_ton, p_source_number_npi, p_destination_number, p_service_type, p_message, p_service_id, 0, 0);
